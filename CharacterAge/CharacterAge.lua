...
frame:SetScript(
  ...
  function(self, event, ...)
    ...
    if (frameUpdaterCounter > GetFramerate() / 6) then
      ...
      gametime:SetText(time2string(number2time(currentPlayed + currentOffset)))
      leveltime:SetText(time2string(number2time(currentLevelPlayed + currentOffset)) .. " @ " .. tostring(currentLevel))
